...
class FCOSDiscriminator_out(nn.Module):
    def __init__(
        ...
        if not base_dis_tower:
            ...
            # self.cls_logits = nn.Conv2d(
            #     ndf * (2**(num_convs - 1)), 1, kernel_size=1, stride=1, padding=1
            # )
            self.cls_logits = nn.Conv2d(
               1, 1, kernel_size=1, stride=1, padding=1
            )
            # self.cls_logits = nn.Linear( ndf * (2**(num_convs-1)), 1  )
    ...
    def forward(self, target, score_map=None, domain='source'):
        ...
        if self.cls_map_pre == 'softmax':
            ...
            if self.output_cls_da:
                # map_list.append(box_cls_map.softmax(1)* centerness_map.sigmoid())

                cls_score_map, cle_score_indx = torch.max(box_cls_map.softmax(1),dim=1)
                map_list.append(cls_score_map.unsqueeze(1)* centerness_map.sigmoid())
        ...
        for i, ctrs in enumerate(output_map):
            root = '/home/wuyang/Pictures/why/'
            ctrs = ctrs.squeeze().detach().cpu().numpy()
            plt.matshow(ctrs)
            plt.title('stride = {}'.format(i))
            h = plt.contourf(ctrs)
            cb = plt.colorbar(h)
            if domain=='source':
                plt.savefig(root+'source_{}.png'.format(i))
            else:
                plt.savefig(root + 'target_{}.png'.format(i))
        ...
        # Forward
        # see(self.dis_tower[0].weight)
        # see(self.dis_tower[0].weight)
        # x = self.dis_tower(output_map)
        # print(x.size())
        # see(x)
        # see(output_map)
        x = self.cls_logits(output_map)
        for i, ctrs in enumerate(x):
            root = '/home/wuyang/Pictures/why/'
            ctrs = ctrs.squeeze().detach().cpu().numpy()
            plt.matshow(ctrs)
            plt.title('stride = {}'.format(i))
            ...
            cb = plt.colorbar(h)
            if domain == 'source':
                plt.savefig(root + 'x_source_{}.png'.format(i))
            else:
                plt.savefig(root + 'x_ target_{}.png'.format(i))
        ...
        #     h = plt.contourf(ctrs)
        #     cb = plt.colorbar(h)
        #     plt.savefig(root+'_scores_{}.png'.format(i))
