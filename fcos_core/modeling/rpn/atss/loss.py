...
class ATSSLossComputation(object):
    def __init__(self, cfg, box_coder):
        ...
        # statistics:
        if cfg.MODEL.ATSS.POSITIVE_TYPE == 'ADAPT_ATSS':
            self.ADAPT_TOPK = self.global_cat_distribution(cfg)





    def global_cat_distribution(self, cfg):

        annos_dir = '/home/wuyang/data/Cityscapes/cocoAnnotations/cityscapes_train_cocostyle.json'
        coco = COCO(annos_dir)
        ...
        for cat in nms:
            ...
            for img_id in imgIds:
                ...
                num += len(annIds)
            cat_ins_dict.update({cat: num})
        cat_ins_array = np.array(list(cat_ins_dict.values()))
        cat_ins_array = np.ceil(np.mean(cat_ins_array) / cat_ins_array * 9)
        ADAPT_TOPK = np.minimum(np.maximum(cfg.MODEL.ATSS.TOPK, cat_ins_array), 17)
        return ADAPT_TOPK
    ...
    def prepare_targets(self, targets, anchors):
        ...
        for im_i in range(len(targets)):
            ...
            if self.cfg.MODEL.ATSS.POSITIVE_TYPE == 'SSC':
            ...
            elif self.cfg.MODEL.ATSS.POSITIVE_TYPE == 'ADAPT_ATSS':
                num_anchors_per_loc = len(self.cfg.MODEL.ATSS.ASPECT_RATIOS) * self.cfg.MODEL.ATSS.SCALES_PER_OCTAVE
                num_anchors_per_level = [len(anchors_per_level.bbox) for anchors_per_level in anchors[im_i]]
                ious = boxlist_iou(anchors_per_im, targets_per_im)
                gt_cx = (bboxes_per_im[:, 2] + bboxes_per_im[:, 0]) / 2.0
                ...
                star_idx = 0
                for level, anchors_per_level in enumerate(anchors[im_i]):
                    candidate_gt_samples = []
                    end_idx = star_idx + num_anchors_per_level[level]
                    distances_per_level = distances[star_idx:end_idx, :]
                    topk_cls = np.minimum(self.ADAPT_TOPK * num_anchors_per_loc,num_anchors_per_level[level])
                    # print(distances_per_level.shape)
                    # print(labels_per_im)
                    # print(topk_cls)

                    for i, cls in enumerate(labels_per_im):
                        topk =int(topk_cls[cls-1])
                        _, topk_idxs_per_level = distances_per_level[:,i].topk(topk, dim=0, largest=False)
                        candidate_gt_samples.append(topk_idxs_per_level + star_idx)
                    candidate_idxs.append(candidate_gt_samples)
                    # tensor([2, 2, 2, 2], device='cuda:0')
                    # array([9., 9., 17., 10., 17., 17., 9., 17.])
                    # topk = min(self.cfg.MOEL.ATSS.TOPK * num_anchors_per_loc, num_anchors_per_level[level])
                    # candidate_idxs.append(topk_idxs_per_level + star_idx)
                    star_idx = end_idx
                candidate_idxs_ins = []
                for i, _ in enumerate(labels_per_im):
                    candidate_idxs_ins.append(torch.cat([candidate_idxs[k][i] for k in range(5)],dim=0))
                is_pos = []
                for i, _ in enumerate(labels_per_im):
                    candidate_ious = ious[candidate_idxs_ins[i], torch.arange(num_gt)[i]]
                    iou_mean_per_gt = candidate_ious.mean(0)
                    iou_std_per_gt = candidate_ious.std(0)
                    iou_thresh_per_gt = iou_mean_per_gt + iou_std_per_gt
                    is_pos.append(candidate_ious >= iou_thresh_per_gt)
                # Limiting the final positive samples’ center to object
                anchor_num = anchors_cx_per_im.shape[0]
                # for ng in range(num_gt):
                #     candidate_idxs_ins[ng] += ng * anchor_num
                e_anchors_cx = anchors_cx_per_im.view(1, -1).expand(num_gt, anchor_num).contiguous().view(-1)
                e_anchors_cy = anchors_cy_per_im.view(1, -1).expand(num_gt, anchor_num).contiguous().view(-1)
                # candidate_idxs = candidate_idxs.view(-1)
                for i, tg_samples_i in enumerate(candidate_idxs_ins):

                    l = e_anchors_cx[tg_samples_i].view(-1) - bboxes_per_im[i, 0]
                    t = e_anchors_cy[tg_samples_i].view(-1) - bboxes_per_im[i, 1]
                    r = bboxes_per_im[i, 2] - e_anchors_cx[tg_samples_i]
                    b = bboxes_per_im[i, 3] - e_anchors_cy[tg_samples_i]
                    is_in_gts = torch.stack([l, t, r, b], dim=1).min(dim=1)[0] > 0.01
                    is_pos[i] = is_pos[i] & is_in_gts

                # if an anchor box is assigned to multiple gts, the one with the highest IoU will be selected.
                ious_inf = torch.full_like(ious, -INF)
                for i in range(num_gt):

                    sample_i_indx = candidate_idxs_ins[i][is_pos[i]]
                    ious_inf[sample_i_indx,i] = ious[sample_i_indx, i]
                # import ipdb
                # ipdb.set_trace()
                # ious_inf = ious_inf.t().contiguous()
                anchors_to_gt_values, anchors_to_gt_indexs = ious_inf.max(dim=1)
                cls_labels_per_im = labels_per_im[anchors_to_gt_indexs]
                cls_labels_per_im[anchors_to_gt_values == -INF] = 0
                matched_gts = bboxes_per_im[anchors_to_gt_indexs]
